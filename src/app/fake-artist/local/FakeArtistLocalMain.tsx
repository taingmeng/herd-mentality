...
export default function FakeArtistLocalMain({
  ...
  const [players, setPlayers] = useLocalStorage<Player[]>("fake-artist.players", []);
  const [playerNames, setPlayerNames] = useLocalStorage<string>(
    "fake-artist.playerNames",
    ""
  );
  const [gameState, setGameState] = useLocalStorage(
    "fake-artist.gameState",
    "new"
  );
  const [passIndex, setPassIndex] = useLocalStorage("fake-artist.passIndex", 0);
  const [revealedFake, setRevealedFake] = useLocalStorage(
    "fake-artist.revealedFake",
    false
  );
  const [revealedWord, setRevealedWord] = useLocalStorage(
    "fake-artist.revealedWord",
    false
  );
  ...
  const startGame = async () => {
    router.refresh();
    const { category, word } = currentQuestion;
    ...
    <>
      {gameState === "new" && (
        <>
          ...
          <textarea
            className="min-w-[75vw] min-h-[50vh] lg:min-h-[25vh] resize p-4 text-center text-gray-900 border border-gray-300 rounded-lg text-2xl bg-transparent dark:border-pink-600 dark:placeholder-grey-400 placeholder-opacity-10 dark:text-white"
            placeholder="Example: Steve, Tony, Natasha"
            onChange={handleChange}
            value={playerNames}
          />
          <label className="block mt-2">Players: {players.length}</label>
          <div className="z-10 w-full max-w-5xl items-center justify-between lg:flex">
            <div className="fixed h-24 bottom-0 left-0 right-0 flex gap-4 p-4 items-end justify-center bg-gradient-to-t from-white via-white dark:from-black dark:via-black">
              <NextButton disabled={!players.length} onClick={startGame} />
            </div>
          </div>
        </>
      )}
      ...
      {gameState === "passing" && (
        <>
          <div className="flex flex-col justify-between items-center min-h-[75vh]">
            ...
            >
              ...
              >
                ...
                <div className="flip-card-back">
                  <h2 className="text-white">{players[passIndex].question}</h2>
            ...
            <div>
              {!isLastPassIndex && (
                <div className="text-center">
                  <label className="block mt-8 text-gray-500">
              ...
              <div className="z-10 w-full max-w-5xl items-center justify-between lg:flex">
                <div className="fixed h-24 bottom-0 left-0 right-0 flex gap-4 p-4 items-end justify-center bg-gradient-to-t from-white via-white dark:from-black dark:via-black">
                  <PrevButton onClick={passPrevious} />
                  <NextButton onClick={passNext} />
                </div>
              </div>
            </div>
          </div>
        ...
        <>
          ...
          {showPlayHint && (
            <div className="text-center">
              ...
              <p className="block mt-4">
                ...
                pointed to the most or has received the same number of votes as
                other artists), the fake artist win.
              </p>
              <p className="block mt-4">
                If the fake artist is caught, they have one chance to guess the
                actual word. If the fake artist&apos;s answer is correct, the
          ...
          <div className="z-10 w-full max-w-5xl items-center justify-between lg:flex">
            <div className="fixed h-24 bottom-0 left-0 right-0 grid grid-cols-3 gap-4 p-4 items-end justify-center bg-gradient-to-t from-white via-white dark:from-black dark:via-black">
              <PrevButton onClick={backToPassing} className="col-span-1">
                Back
              </PrevButton>
              {!revealedFake && (
                <NextButton onClick={revealFakeArtist} className="col-span-2">
                  Reveal Fake Artist
                </NextButton>
              )}
              {revealedFake && !revealedWord && (
                <NextButton onClick={revealTheWord} className="col-span-2">
                  Reveal The Word
                </NextButton>
              )}
              {revealedFake && revealedWord && (
                <NextButton onClick={newGame} className="col-span-2">
                  New Game
                </NextButton>
              )}
            </div>
