...
const dataPath = `src/app/chameleonz/data`;
    ...
    fs.readFile(absoluteFilePath, "utf8", (error, data) => {
      ...
      for (let i = 1; i < lines.length; i++) {
        if (lines[i].trim().length > 0) {
          if (!lowercaseSet.has(lines[i].trim().toLowerCase())) {
            lowercaseSet.add(lines[i].trim().toLowerCase());
            set.add(lines[i].trim());
          }
        }
      }
      const newLines = Array.from(set).sort();
      fs.writeFile(absoluteFilePath, "word\n" + newLines.join("\n") + "\n", (error) => {
